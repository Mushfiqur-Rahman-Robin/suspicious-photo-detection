...
- Unit + integration + reproducibility test suite (198 tests, ~94% coverage) covering
...
- `write_up.md` now carries a **`## Validation`** section: the synthetic-golden
  gate results, the reproducibility claim (byte-identical re-run via fixed seed
  + content-addressed cache), and a **quantitative qualitative review of real
  flags** (SPEC §16) - 11 sampled flags across 7 outlets, each verified to be
  closer to a different outlet's photos than to its own (cross-outlet cosine
  0.18-0.84 vs own-outlet mean 0.01-0.54), i.e. photos of other storefronts.
  The review is generated by `reporting/write_up.py` so the write-up stays
  consistent with any re-run.
